...
void audio_callback(void* userdata, Uint8* stream, int len) {
    ...
    static int pkt_data_pos = 0;

    int audio_data_size = 0;
    int len_to_copy;

    while (len > 0) {
        if (pkt_data_pos >= pkt.size) {
            if (1 < 0) {
                return;
            }
            //// Read the next audio packet
            //if (av_read_frame(audio_ctx->opaque, &pkt) < 0) {
            //    return;
            //}

            //// Check if the packet belongs to the audio stream
            //if (pkt.stream_index != audio_stream_index) { // Use the correct audio_stream_index here
            //    av_packet_unref(&pkt);
            //    continue;
            //}
            ...
            //if (avcodec_receive_frame(audio_ctx, frame) < 0) {
            //    av_packet_unref(&pkt);
            //    continue;
            //}
        ...
        len_to_copy = len;
        if (len_to_copy > pkt.size - pkt_data_pos) {
            len_to_copy = pkt.size - pkt_data_pos;
        }

        // Mix audio data and adjust volume
        SDL_MixAudioFormat(stream, pkt.data + pkt_data_pos, AUDIO_S16SYS, len_to_copy, volume_level);

        pkt_data_pos += len_to_copy;
        stream += len_to_copy;
        len -= len_to_copy;
    }
}
